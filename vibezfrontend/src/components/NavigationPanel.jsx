...
const NavButton = ({ icon, label, badge = false, onClick }) => (
    <li>
        <button onClick={onClick} className="flex items-center justify-between text-gray-400 hover:text-white w-full">
            <div className="flex items-center space-x-3">
                {icon}
                <span className="font-semibold">{label}</span>
            </div>
                            ...
                            <div className="ml-4 mt-2 p-3 bg-gray-900 rounded-lg shadow-lg space-y-2">
                                ...
                                <ul className="space-y-1 max-h-64 overflow-y-auto custom-scrollbar pr-1">
                                    {notifications && notifications.length > 0 ? (
                                        ...
                                        <p className="text-gray-500 text-sm p-2">Brak powiadomień.</p>
                                    )}
                                </ul>
                            </div>
                        )}
