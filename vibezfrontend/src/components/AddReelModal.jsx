...
export default function AddReelModal({ user, onClose, onReelAdded }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setUploadProgress('Processing video on server...');
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50" onClick={onClose}>
            <div className="bg-gray-900 border border-gray-700 p-6 rounded-lg shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <FileInput
                        label="Video File (AVI/MOV/etc will be converted to MP4 on server)"
                        accept="video/*"
                    ...
                    {uploadProgress && (
                        <div className="bg-blue-900 border border-blue-700 text-blue-200 p-3 rounded-lg text-sm">
                            {uploadProgress}
                        </div>
                    )}
                    ...
                    <div className="flex justify-end pt-4">
                        <button
                            ...
                            onClick={onClose}
                            disabled={uploading}
                            className="mr-4 py-2 px-4 text-sm font-medium text-gray-400 hover:text-white disabled:opacity-50"
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            disabled={uploading}
                            className="py-2 px-6 bg-white text-black font-semibold rounded-lg hover:bg-gray-200 disabled:bg-gray-500 disabled:cursor-not-allowed"
                        >
